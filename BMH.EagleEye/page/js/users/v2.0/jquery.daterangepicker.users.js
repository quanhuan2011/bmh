    ...
    $.fn.dateRangePicker = function (opt) {
        ...
        function init_datepicker() {
                ...
                if (url.indexOf('?') > 0) {
                    if (url.length == url.indexOf('?') + 1) {
                        ...
                        if (key != null) {
                            ...
                            url = url.replace(keyStr, inKey + "=" + inVal);
                        }
                        else {
                            url = url + "&" + inKey + "=" + inVal;
                        }
            ...
            box.find('.apply-btn').click(function () {
                ...
                var starttimes = getDateString(new Date(opt.start)).replace(/-/g, "");
                var endtimes = getDateString(new Date(opt.end)).replace(/-/g, "");
                //var dimensionType = "day";

                if (endtimes == 'Invalid date') {
                    endtimes = starttimes;
                } else {
                    endtimes = getDateString(new Date(opt.end)).replace(/-/g, "")
                }
                var url = location.href;
                url = getMatchUrl(url, "starttime", starttimes);
                url = getMatchUrl(url, "endtime", endtimes);
                location.href = url;
        ...
        function createDom() {
            ...
            html += '</div>\
				...
				+ '<div class="dateArea"><div class="date-start">开始日期</div><div class="date-end">结束日期</div></div>'
            ...
            html += '</div>';
            $('.dateArea').before($('.footer'));
            $('.month-wrapper').after($('.drp_top-bar'));
        ...
        function changeColor() {
            if ($('.date-picker-wrapper a').hasClass('on')) {

            } else {
                $('.date-picker-wrapper a:first').addClass('on')
            }

            $('.date-picker-wrapper').on('click', 'a', function () {
                $('.prev-buttons').find('a').removeClass('on');
                $(this).addClass('on').siblings('a').removeClass('on');
            })
            $('.prev-buttons').find('a').click(function () {
                $(this).addClass('on').siblings('a').removeClass('on').parent().prevAll().removeClass('on');
            })
            $('a[shortcut="day,-1"]').text('昨天');
            $('a[shortcut="day,-0"]').text('今天');
            $('a[shortcut="day,-7"]').text('最近7天');
